...
#include <QFileInfo>        //информация о пути/имени файла
#include <QTextStream>      //чтение текста из файла
#include <QRegularExpression> //для парсинга

OptimizationRunner::OptimizationRunner(QObject* parent) : QObject(parent) {
    //создаём QProcess. this — автоматическое уничтожение, когда уничтожится OptimizationRunner
    //(система родитель-потомок Qt, как и с виджетами)
    process = new QProcess(this);

    //SeparateChannels — разделяем stdout и stderr, чтобы можно было по-разному
    //их подсвечивать/обрабатывать при желании
    process->setProcessChannelMode(QProcess::SeparateChannels);

    //подключение сигналов QProcess
    connect(process, &QProcess::readyReadStandardOutput, this, [this] {
        //readAllStandardOutput() — читает весь накопившийся вывод программы с последнего раза
        //fromLocal8Bit — превращает байты из консоли в правильную QString (из-за кириллицы)
        QString output = QString::fromLocal8Bit(process->readAllStandardOutput()); //← СОХРАНЯЕМ В ПЕРЕМЕННУЮ
        
        //emit — ключевое слово Qt, которым мы "испускаем" сигнал
        //после этого все, кто подписан (через connect) на outputLine, получат вызов своего слота
        emit outputLine(output);

        //ПАРСИНГ ДАННЫХ ДЛЯ ГРАФИКА
        //ищем строки вида "Iteration 5: value = 0.42" или "Iteration 5 value=0.42"
        QRegularExpression regex(R"(Iteration\s+(\d+)[:\s]+value\s*=\s*([\d.]+))");
        QRegularExpressionMatchIterator it = regex.globalMatch(output);
        while (it.hasNext()) {
            QRegularExpressionMatch match = it.next();
            int iter = match.captured(1).toInt();        //номер итерации
            double value = match.captured(2).toDouble(); //значение функции
            emit iterationData(iter, value);             //отправляем сигнал для графика
        }
    });
    ...
    //указать компилятору, какую именно версию сигнала мы имеем в виду —
    //для этого используется QOverload
    connect(process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            this, [this](int code, QProcess::ExitStatus) { emit processFinished(code); });
...
QStringList OptimizationRunner::buildArguments(const QStringList& methods,
    ...
    return args;
    //если methods = ["gradient_descent", "newton"], functions = ["rosenbrock"], noAnalysis = true,
    //то вернётся: ["-m", "gradient_descent", "newton", "-f", "rosenbrock", "-s"]
}
...
void OptimizationRunner::run(const QString& exePath, const QStringList& methods,
    ...
    //важно, потому что ваша программа создаёт app.log рядом с собой,
    //а также ищет файлы через относительные пути — если не выставить рабочую
    //директорию правильно, программа может искать файлы не там
    process->setWorkingDirectory(QFileInfo(exePath).absolutePath());

    //start() запускает процесс АСИНХРОННО — то есть эта функция сразу возвращает управление,
    //а сама программа работает "в фоне". Мы узнаём о её выводе и завершении
    //через сигналы, которые подключили в конструкторе выше
...
QString OptimizationRunner::readAppLog(const QString& exePath) const {
    QString logPath = QFileInfo(exePath).absolutePath() + "/app.log";
    QFile file(logPath);
